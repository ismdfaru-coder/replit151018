...
/**
 * @fileOverview A conversational AI flow for booking flights using HuggingFace DeepSeek.
 *
...
function createPrompt(conversationHistory: string, availableFlights?: string): string {
  let prompt = `You are an expert travel agent AI. Your goal is to have a conversation with the user to gather all the necessary information to find and book a flight.
...
You must respond with a valid JSON object with this exact structure:
{
  ...
  "isFlightDetailsComplete": true or false,
  "flightDetails": {
    "destination": "destination if available",
...
async function flightBookingAssistantFlow(input: FlightBookingInput): Promise<FlightBookingOutput> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const prompt = createPrompt(
        ...
        input.availableFlights ? JSON.stringify(input.availableFlights) : undefined
      );
      ...
      try {
        ...
        return {
          ...
          isFlightDetailsComplete: parsed.isFlightDetailsComplete || false,
  ...
  if (lastError?.status === 401 || lastError?.status === 403) {
    return {
      reply: "🔑 There's an API authentication issue. Please check the Hugging Face API key configuration.\n\n✈️ Flight search functionality is still working!",
      isFlightDetailsComplete: false,
    };
  }
