        ...
        const fetchQueryAndFlights = async () => {
            ...
            try {
                ...
                if (parsed.destination && parsed.dates) {
                    ...
                    if (flightData.status === 'ok' && flightData.flights && flightData.flights.length > 0) {
                        // Success with flights
                        setFlights(flightData.flights);
                        setError('');
                    } else if (flightData.status === 'no_results') {
                        // Google Flights explicitly returned no results
                        setError(flightData.message || 'No flights found for your search criteria. Try different dates or destinations.');
                        setFlights([]);
                    } else if (flightData.status === 'parse_error') {
                        // Failed to parse Google Flights data
                        setError(flightData.message || 'Unable to parse flight data. The page structure may have changed.');
                        setFlights([]);
                    } else if (flightData.status === 'api_failure') {
                        // API or connection failure
                        setError(flightData.message || 'Unable to connect to flight search service. Please try again later.');
                        setFlights([]);
                    } else if (flightData.error) {
                        // Legacy error handling for backward compatibility
                        setError(`Flight Search Error: ${flightData.error}`);
                        setFlights([]);
                    } else {
