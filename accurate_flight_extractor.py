...
def extract_accurate_flight_data(soup, origin: str, destination: str) -> List[Dict]:
    ...
    # First check if Google Flights explicitly shows "no flights found"
    ...
    if any(indicator in html_text for indicator in no_flights_indicators):
        ...
        return []  # This is a genuine "no flights" from Google, not a parsing error
...
def extract_accurate_airline(text: str) -> str:
    ...
    # Look for exact airline name matches (prioritize longer matches)
    for pattern, name in screenshot_airlines:
        if pattern in text_lower:
            # Additional validation: check if this is actually an airline mention
            context_around = text_lower
            if any(word in context_around for word in ['operated', 'airline', 'flight', 'airways']):
                return name
...
def extract_accurate_duration(text: str) -> str:
    ...
    # Enhanced patterns from real Google Flights HTML
    duration_patterns = [
        r'(\d{1,2})\s*h\s*r?\s*(\d{1,2})\s*m(?:in)?',     # "13hr 5m", "14hr 5min"
        r'(\d{1,2})\s*hr\s*(\d{1,2})\s*m(?:in)?',         # "13hr 5m", "13hr 5min"
        r'(\d{1,2})\s*hours?\s*(\d{1,2})\s*min(?:utes)?',  # "13 hours 5 minutes"
        r'(\d{1,2})\s*:\s*(\d{2})',                       # "13:05" format
        r'Duration[^\d]*(\d{1,2})\s*h[^\d]*(\d{1,2})\s*m', # "Duration: 13h 5m"
        r'(\d{1,2})h\s+(\d{1,2})m',                       # "13h 5m" (exact Google format)
        r'(\d{1,2})\s*hr\s+(\d{1,2})\s*m'                 # "13 hr 5 m"
    ]
...
def extract_accurate_times(text: str, idx: int) -> tuple[str, str]:
    ...
    # Enhanced time patterns from Google Flights HTML
    time_patterns = [
        r'(\d{1,2}:\d{2})\s*(?:AM|PM|am|pm)?',
        r'Departure[^\d]*(\d{1,2}:\d{2})',
        r'Arrival[^\d]*(\d{1,2}:\d{2})',
        r'Depart[^\d]*(\d{1,2}:\d{2})',
        r'Arrive[^\d]*(\d{1,2}:\d{2})',
        r'(\d{1,2}:\d{2})\s*[+-]\d',  # Times with timezone indicators
        r'(\d{1,2}:\d{2})\s*(?:departure|arrival)',  # "14:35 departure"
        r'(?:departure|arrival)\s*(\d{1,2}:\d{2})'   # "departure 14:35"
    ]
...
def extract_real_flight_details(container_text: str, price: int, idx: int, origin: str, destination: str) -> Optional[Dict]:
    ...
    try:
        ...
        print(f"❌ Real flight details extraction error: {e}", file=sys.stderr)
        return None